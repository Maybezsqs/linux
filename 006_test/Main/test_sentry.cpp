...
void ImgProdCons::init()
{
    ...
    //Initialize camera
    _videoCapturePtr->open(1,6); // 0 works fine for small panel , if you meet problem opening the camera, try change this
//    _videoCapturePtr->setVideoFormat(640, 480, true);
        _videoCapturePtr->setVideoFormat(1280, 720, true);
    _videoCapturePtr->setExposureTime(100/*100*/);
...
void ImgProdCons::consume()
{
    ...
    time_t t;
    time(&t);
    const string fileName = "/home/zsq/videos/" + to_string(t) + ".avi";
    //writer.open(fileName, CV_FOURCC('M', 'J', 'P', 'G'), 60, Size(640, 480));
...
//        cout<<1<<endl;
        ...
        if(armorFlag == ArmorDetector::ARMOR_LOCAL || armorFlag == ArmorDetector::ARMOR_GLOBAL)
        {
            ...
            for(int j=0;j<4;++j)
            {
                line(test1,armorVertex[j],armorVertex[(j+1)%4],Scalar(200,0,150),10);
            }
            imshow("test",test1);
